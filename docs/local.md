...
12. 填上前面获取的令牌
- 若要同步到 Microsoft Todo（可选 ACMOJ 作业），按以下格式填写
```
{"CanvasToken":"这里填上你的 Canvas 令牌","AcmOjToken":"这里填上你的 ACMOJ 令牌","GraphToken":"这里填上你的 Graph 令牌"}
```

- 若要同步到滴答清单（可选 ACMOJ 作业），按以下格式填写
```
{"CanvasToken":"这里填上你的 Canvas 令牌","AcmOjToken":"这里填上你的 ACMOJ 令牌","DidaCredential":{"phone":"这里填上你的滴答清单登录手机号","password":"这里填上你的滴答清单登录密码"}}
```
